...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpstr, int nCmdShow) {
	...
	HWND MainWindow = CreateWindowEx(WS_EX_CLIENTEDGE | WS_EX_TOPMOST, "NTFSCleaner", "Ntfs-Cleaner (v0.5 Alpha)", WS_OVERLAPPED | WS_VISIBLE | WS_SYSMENU, CW_USEDEFAULT, CW_USEDEFAULT, 800, 500, NULL, NULL, hInstance, NULL);
	...
	HWND MenuButton = CreateWindowEx(0, "BUTTON", "Menu", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON | BS_FLAT, 310, 155, 100, 30, MainWindow, (HMENU)1005, hInstance, NULL);
	...
	if (SafeModeTrigger == 1) {
		UpdateLog(MainWindow, "SafeMode Enable.", RGB(255, 255, 255));
	}
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(MainWindow, LogFile, "Start NtfsCleaner!");
	}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
		...
		case 1001:
			...
			MessageBox(
				hwnd,
				"V0.5, Created By Spectre2Ux.\nNew Func:\n- Now every step is logged.\n- NEW MENU!\n- Improved UI\n- Error handling\n- Func Destruct by Process Inject\n- SafeMode\n- logging systems", "Reference (V0.5)",
				MB_ICONINFORMATION);
		...
		case 1010:
			...
			if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
				WriteLog(hwnd, LogFile, "SafeMode Triggered, You do not have permission to perform this function because Safe Mode is enabled.");
			}
			break;
			}
	...
	case WM_PAINT: {
		...
		TextOut(hdc, 260, 350, "V0.5 Alpha, bugs may be present.", (int)strlen("V0.5 Alpha, bugs may be present."));
...
void OpenProcessByNtfsToken(HWND hwnd) {
	...
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Start Open Test App...");
	}
	...
	if (!CreateProcess(TestPath, NULL, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &TestParameter, &TestInformation)) {
		...
		if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
			WriteLog(hwnd, LogFile, "Error, CreateProcess Func.");
		}
...
void DestructByProcessInject(HWND hwnd) {
	...
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Start Access To DestructByProcessInject");
	}
	...
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Start Find Target Process...");
	}
	...
	if (ProcessList == INVALID_HANDLE_VALUE) {
		...
		if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
			WriteLog(hwnd, LogFile, LogTextOnParameter);
		}
		return;
	}
	...
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Found Target Process.");
	}

	Sleep(200);
	UpdateLog(hwnd, "Search Memory To Target Process.", RGB(255, 255, 255));
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Search Memory To Tagret Process.");
	}
	...
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Attempt or Open Target Process.");
	}
	...
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "SuccessFully Open Target Process!");
	}

	Sleep(200);
	UpdateLog(hwnd, "Create Path To Dll...", RGB(255, 255, 255));
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Create Path To Dll...");
	}
	...
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Start Memory Allocation...");
	}
	...
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Successfully Memory Allocation!");
	}
	...
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Start Memory Writing...");
	}
	...
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Successfully! Memory Writed.");
	}
	...
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Create Handle To Load Library...");
	}
	...
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Handle To Load Library Successfully Created!");
	}
	...
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Start Remote Inject...");
	}
	...
	if (WriteFileStatus && LogFile != INVALID_HANDLE_VALUE) {
		WriteLog(hwnd, LogFile, "Successfully Injected.");
	}
...
void CreateFileToWriteLog(HWND hwnd) {
	LogFile = CreateFile("NtfsCleaner_v0.5_Log.txt", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	if (SafeModeTrigger == 1) {
		WriteLog(hwnd, LogFile, "SafeMode Enable.");
	}
	else {
		WriteLog(hwnd, LogFile, "SafeMode Disable.");
	}

	Sleep(300);
	char DirectoryPath[MAX_PATH];
	DWORD CurrentLogFileDirectory = GetCurrentDirectory(MAX_PATH, DirectoryPath);
	strcat(DirectoryPath, "\\NtfsCleaner_v0.5_Log.txt");
...
void DeleteLogFile(HWND hwnd) {
	...
	strcat(DeleteFilePathBuffer, "\\NtfsCleaner_v0.5_Log.txt");
