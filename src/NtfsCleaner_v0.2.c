...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpstr, int nCmdShow) {
	...
	HWND MainWindow = CreateWindowEx(WS_EX_CLIENTEDGE, "NTFSCleaner", "Ntfs-Cleaner (v0.2 alpha)", WS_OVERLAPPED | WS_VISIBLE | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX, CW_USEDEFAULT, CW_USEDEFAULT, 500, 500, NULL, NULL, hInstance, NULL);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
		if (LOWORD(wParam) == 1000) {
			...
			MessageBox(hwnd, "V0.2, Created By Spectre2Ux.\nNew Func:\n\n- Reference button.\n- Operation log on Lite Button.\n- Added colors.", "Reference", MB_ICONINFORMATION);
			break;
		}
	...
	case WM_PAINT: {
		...
		SetTextColor(hdc, RGB(255, 255, 255));
		TextOut(hdc, 140, 260, "Clearing Windows Log Mods:", (int)strlen("Clearing Windows Log Mods:"));

		SetBkColor(hdc, RGB(255, 255, 255));
		SetTextColor(hdc, RGB(0, 0, 0));

		TextOut(hdc, 130, 380, "# <_O Created By Spectre2Ux", (int)strlen("# <_O Created By Spectre2Ux"));
		TextOut(hdc, 120, 410, "V0.2 Alpha, bugs may be present.", (int)strlen("V0.2 Alpha, bugs may be present."));
